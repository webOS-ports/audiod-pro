...
{
    /* Filled in by the card-profile routing work. Until then the state machine
     * is exercised and logged, but nothing is pushed to the hardware. */
    PM_LOG_INFO(MSGID_PALM_LEGACY_MANAGER, INIT_KVCOUNT,
        "applyCallRouting: mode=%d route=%d (routing backend not yet wired)",
        (int) mode, (int) route);
    return true;
}
