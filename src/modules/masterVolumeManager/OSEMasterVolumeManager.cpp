    ...
    if (settingsObj.isValid() && settingsObj.isObject())
    {
        ...
        if (outputInfo.isValid() && outputInfo.isArray())
        {
            ...
            for( auto devices : outputInfo.items())
            {
                ...
                if (devices["volume"].asNumber<int>(volume)== CONV_OK)
                {
                    PM_LOG_DEBUG("volume read");
                }
                if (devices["deviceCounter"].asNumber<int>(deviceNum)== CONV_OK)
                {
                    PM_LOG_DEBUG("deviceCounter read");
                }
    ...
    if(mConnectedDevicesList.size()>0)
    {
        for (auto &it:mConnectedDevicesList)
        {
            PM_LOG_INFO(MSGID_CLIENT_MASTER_VOLUME_MANAGER, INIT_KVCOUNT,"connected afterwards it.deviceName : %s, it.deviceNameDetail : %s, it.isOutput :%d", \
                it.deviceName.c_str(), it.deviceNameDetail.c_str(), it.isOutput);
...
void OSEMasterVolumeManager::sendDataToDB()
{
    ...
    if (!LSCall(sh, SETSETTINGS, finalString.stringify().c_str(),nullptr,nullptr,nullptr, &lserror))
...
bool OSEMasterVolumeManager::updateMasterVolumeInMap(std::string deviceName, int displayId,int volume, bool isOutput)
{
    ...
    if (isInternalDevice(deviceName,isOutput))
    {
        ...
        for(auto &it: deviceList)
        {
            ...
            {
                PM_LOG_INFO(MSGID_CLIENT_MASTER_VOLUME_MANAGER, INIT_KVCOUNT, "updateMasterVolumeInMap updating volume of %s to %d", deviceName.c_str(), volume);
                it.volume = volume;
                break;
            }
        }
    ...
    else
    {
        ...
        for(auto &it: deviceList)
        {
            ...
            {
                PM_LOG_INFO(MSGID_CLIENT_MASTER_VOLUME_MANAGER, INIT_KVCOUNT, "updateMasterVolumeInMap updating volume of %s to %d", modDevice.c_str(), volume);
                it.volume = volume;
                break;
            }
        }
...
{
    PM_LOG_INFO(MSGID_CLIENT_MASTER_VOLUME_MANAGER, INIT_KVCOUNT,\
        "eventActiveDeviceInfo deviceName:%s display:%s isConnected:%d isOutput:%d isactive : %d", \
        deviceName.c_str(), display.c_str(), (int)isConnected, (int)isOutput, (int)isActive);
...
void OSEMasterVolumeManager::deviceDisconnectOp(std::string deviceName, std::string deviceNameDetail,bool isOutput)
{
    ...
    else
    {
        ...
        getVolumeFromDB(deviceNameDetail, isOutput, false, found);
        if (found)
        {
...
void OSEMasterVolumeManager::eventServerStatusInfo(SERVER_TYPE_E serviceName, bool connected)
{
    ...
    if (connected && serviceName == eSettingsService2)
    {
        ...
        if (!LSCallOneReply (sh, GETSETTINGS, payload.c_str(), VolumeFromSettingService, ptrOSEMasterVolumeManager, nullptr, &lserror))
            lserror.Print(__FUNCTION__, __LINE__);
    }
}
